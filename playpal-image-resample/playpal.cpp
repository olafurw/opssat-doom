...
closest_color_result closest_color(unsigned char r, unsigned char g, unsigned char b)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        if (i == 0 || dist < result.distance)
        {
            result.distance = dist;
            result.index = i;
            result.r = color_index[i][0];
            result.g = color_index[i][1];
            result.b = color_index[i][2];
        }
    }
...
void update_color_with_average(int index, const std::vector<std::array<unsigned char, 3>>& colors)
{
    ...
    color_index[index][0] = r / colors.size();
    color_index[index][1] = g / colors.size();
    color_index[index][2] = b / colors.size();
}

 unsigned char color_index[256][3] = {
    { 0, 0, 0 },
    { 31, 23, 11 },
    { 23, 15, 7 },
    { 75, 75, 75 },
